...
export default function Auto({setCircuito}) {
    ...
    const [VS, setVS] = useState(-1000)
    const [RS, setRS] = useState(-1000)

    let verify = [-1000, -1000, -1000, -1000, -1000, -1000, -1000, -1000, -1000, -1000, -1000, -1000]
    // let verifyName = ["ID", "IDSS", "Vp", "VGS", "VDD", "RD", "VDS", "VD", "VG", "IS", "VS", "RS"]
    ...
    const Calcula =  () => {
        for(let x = 0; x < 7; x++){
            console.log(x)
                ...
                if(verify[4] === -1000){
                    if(VDS !== -1000 && ID !== -1000 && RD !== -1000 && RS !== -1000){
                        let k = VDS +  RD * ID + RS * ID;
                        setVDD(k)
                        verify[4] = VDD;
                    }else{
                        if(VDS !== -1000 && VD !== -1000 && VS !== -1000){
                            let k = (VDS + VD + VS);
                            setVDD(k)
                            verify[4] = VDD;
                        }
                ...
                if(verify[5] === -1000){
                    if(VDS !== -1000 && VDD !== -1000 && ID !== -1000 && VS !== -1000){
                        let k = (VDD - VDS - VS) / ID;
                        setRD(k)
                        verify[5] = RD;
                    }else{
                ...
                if(verify[6] === -1000){
                    if(VDD !== -1000 && ID !== -1000 && RD !== -1000 && VS !== -1000){
                        let k = VDD - ID * RD - VS;
                        setVDS(k)
                        verify[6] = VDS
                    }else{
                        if(VDD !== -1000 && ID !== -1000 && RD !== -1000 && VS !== -1000 && RS !== -1000){
                            let k = VDD - ID * RD - ID * RS;
                            setVDS(k)
                            verify[6] = VDS
                        }
                    }
                ...
                if(verify[11] === -1000){
                    if(VDD !== -1000 && VDS !== -1000 && VD !== -1000 && ID !== -1000 ){
                        let k = (VDD - VD -VDS) / ID;
                        setRS(k)
                        verify[11] = RS
                    }else{
    ...
    <div className="content">
      <Header />
        <div className="imageContent">
            <img className="ContentImg" onClick={setViewOne} src="./cc.png" alt="" />
            <img className="ContentImg addColor" src="./polari.png" alt="" />
            <img className="ContentImg" src="./paralelo.png" alt="" />
        </div>
        <div className="contentVariable">
            <div className="leftVariable">
                <p className="paragrafoAling">ID = IS;</p>
                <p className="paragrafoAling">RG = M ohm; IG = 0;</p>
            </div>
            <div className="rightVariable">
                <p className="paragrafoAling">VGS = -VS; VS = RS * ID</p>
                <p className="paragrafoAling">VDD = VD + VDS + VS;</p>
            </div>
        </div>
        {type === 'quadrado' ? (
            ...
            <div className="contetForm">
                <div className="form">
                    ...
                    <div className="allinpsContent topmarginForm">
                        ...
                        <input className="inp" onChange={(e)=> setVS(e.target.value)} type="number"  placeholder="VS" />
                        <input className="inp" onChange={(e)=> setRS(e.target.value)} type="number"  placeholder="RS" />
                    </div>
                ...
                <div className="result">
                    <div className="resultData">
                        ...
                        <p className="datap">VS = {VS === -1000 ? " " : VS} V</p>
                        <p className="datap">RS = {RS === -1000 ? " " : RS} ohm</p>
                    </div>
                </div>
